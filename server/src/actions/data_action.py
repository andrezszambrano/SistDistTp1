...
class DataAction(Action):
    ...
    def perform_action_(self, _finished_bool, weather_communication_handler, duplicated_stations_communication_handler):
        if self._data_type == Protocol().WEATHER_DATA:
            weather_communication_handler.send_data_to_weather_process(self._data)
        else:
            duplicated_stations_communication_handler.send_data_to_duplicated_station_process(self._data)
